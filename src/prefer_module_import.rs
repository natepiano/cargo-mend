...
/// Walk a proc-macro2 token stream looking for bare identifiers that match
/// imported function names. Skips identifiers preceded by `::` (already
/// part of a qualified path).
    ...
    for tt in tokens.clone() {
        match tt {
            ...
            proc_macro2::TokenTree::Punct(ref punct) => {
                if punct.as_char() == ':' {
                    if prev_colon_joint {
                        // Second `:` completes `::` — next ident is qualified
