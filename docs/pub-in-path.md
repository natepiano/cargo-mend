...
- **Key files:** (all verified; where the design's own line refs were stale the corrected value is given)
  ...
  - `src/compiler/constants.rs` — `FINDINGS_SCHEMA_VERSION` at `:65` (currently `19`; Phase 7 bumped it from `18`)
...
**Files:**
- `src/compiler/visibility/policy.rs` — **no `src/` edit is expected here; this behavior already shipped in Phase 7.** `classify_suspicious_pub` (`policy.rs:50-66`) computes a `required_path` from `PubInPath::Required` + `VisibilitySyntax::Public` + a resolved chain reach, then gates `basic_suspicious_pub_allowance`, `assess_parent_facade_usage` (now `:465`), **and** the `ShallowPrivatePolicy` allowance on `required_path.is_none()`. `required_setting_reviews_bare_pub_behind_restricted_facade` (`tests/diagnostics/forbidden_pub_in_crate.rs:137`) already asserts `SuspiciousPub` at `"required"` and silence at `"permitted"`. Edit `src/` only if the **default** flips — in which case the five config sites listed below apply. This phase's remaining work is the hana conversion, the default decision, and the CHANGELOG entry
...
**There is a third annotation parser and it must stay separate.** Phase 7 added `canonical_pub_in_boundary` (`policy.rs:361`), a `proc_macro2::TokenStream` walk that resolves a written `pub(in ...)` path to a `crate::`-rooted boundary, called from both `record.rs:635` and `load.rs:137`. It answers a different question from the other two — *which module does this path name*, versus `fixes/field_visibility.rs:89` (*how many bytes is the annotation*) and `annotation.rs` (*which `VisibilitySyntax` is this*). Do not fold it into the shared span parser.
