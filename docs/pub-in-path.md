...
- **Key files:** (all verified; where the design's own line refs were stale the corrected value is given)
  ...
  - `src/compiler/constants.rs` — `FINDINGS_SCHEMA_VERSION` at `:65` (currently `21`; Phase 7 bumped it `18` → `19`, Phase 9 took it to `21` for the typed visibility-constraint facts)
  ...
  - `src/fixes/unused_pub.rs` — `scan_from_report` at `:15`. **It does *not* do a raw `"pub "` string search:** it calls a purpose-built `bare_pub_annotation_byte_len` (`:36`, defined `:56-62`) that **deliberately refuses `pub(`**. That refusal is behavior, not an oversight — swapping in `field_visibility.rs`'s permissive parser would make `--fix` start stripping restricted annotations
  - `src/fixes/narrow_pub_crate.rs` — `scan_from_report` at `:15`; `:32` is the actual raw `line_text.find("pub ")` search
...
- **Invariants:**
  ...
  - **`FINDINGS_SCHEMA_VERSION` is `21`** (`src/compiler/constants.rs:65`; Phase 7 bumped it `18` → `19` when findings gained `visibility_annotation`, Phase 9 took it to `21` when the typed visibility-constraint facts got their own report collection). **A bump silently blanks every stale report** — `load.rs` rejects on version mismatch, so a repo whose `cargo check` is fully cached emits nothing fresh, has its old reports discarded, and prints `No findings.` indistinguishably from a real pass. Any self-policy run or manual scan must `rm -rf target/mend-findings` and force a recompile first, or it proves nothing. Any change to the persisted shape *or to the meaning of stored values* requires a bump; `load.rs:166-168` rejects on version, analysis fingerprint, or config fingerprint mismatch. Prior-schema reports are rejected, never partially loaded.
  ...
  - **Every diagnostics fixture that depends on a `[visibility]` setting must write its own project `mend.toml`.** Phase 6 moved `allow_prelude_pub_mod` — and shipped `pub_in_path` — onto project>global precedence, so a fixture that writes no `mend.toml` now inherits **the developer's real machine-global config** and its result depends on who runs it. This is not hypothetical: no fixture in `tests/diagnostics/forbidden_pub_in_crate.rs` writes one, and neither does the all-diagnostics fixture in `rendering.rs`, so once Phase 7 makes `pub_in_path` load-bearing a developer whose global config says `pub_in_path = "required"` gets extra `suspicious_pub` findings and breaks **both** `assert_eq!(…, 16)` gates on unchanged source. Phase 6 already fixed its own two prelude fixtures this way; Phase 7 owns pinning the pre-existing setting-dependent ones, and Phases 9 and 10 must pin every fixture they add.
...
**Phases 6, 8, 10** — Phase 6 is fully self-contained and needed no body edit; only two Delegation Context line refs were off. Phase 8 gained a seventh README edit and four more CHANGELOG cases (seven → eleven), covering the dynamic facade wording plus three behavior changes from earlier phases. Phase 10's "~51 sites" lower bound was raised again — Phase 4 added roughly a dozen more of the exact shape `Required` converts.
...
- Phase 10 (`required` mode) needs no further config work; the three-state key and its precedence are complete and a repo can now pin `required` in its own `mend.toml`.
...
- **Phase 10's default-flip branch gained its real Files list** — five config sites, plus the recorded fact that reconcile writes `"permitted"` onto every upgraded user's disk and would override a flipped default.
- **Stale line refs corrected across the doc.** Phase 5 landed 39 files and never got a review pass, so `record.rs` (now 671 lines), `policy.rs` (753), and `rendering.rs` (1543) had drifted by up to 80 lines in Phases 7/9/11. Two specifics that would have caused real errors: the `suggestion: None` Phase 7 cites as one site is now **two** (`record.rs:608` and `:634`), and `rendering.rs` has **two** 16-findings assertions (`:404`, `:522`) where three gates said one (`:399`).
...
- No remaining phase is redundant or already satisfied by Phase 6; Phase 6 was config-only and each of Phases 7–11 keeps its full job.
...
- `src/reporting/cargo_json.rs` — machine output resolves in the **opposite** order, `inline_help_text(..).or_else(custom_inline_help_text)` (`:209-211`), so today `cargo mend --message-format=json` would print `consider using: pub(super)` while the terminal printed the boundary annotation for the same finding. `rustc_diagnostic` (`:154-167`) sidesteps the choice by emitting **both** as separate `help` children, so a consumer sees two contradictory suggestions on one finding. Make both renderers resolve identically and make `rustc_diagnostic` emit exactly one `help` child per finding. Neither this phase nor Phase 10 listed these two files before this decision, so as previously written neither could fix it.
...
- **Phases 8, 9, 10, 11** — every acceptance gate now requires clearing `target/mend-findings` and forcing a recompile before the self-policy run. After a schema bump a fully cached `cargo check` prints `No findings.` indistinguishably from a real pass; this happened for real during Phase 7's smoke test.
...
**Implications for remaining phases:**
- Phase 11's defect-1 instruction to unify `def_path_is_descendant` is **already satisfied** — one copy now lives at `visibility/policy.rs`, re-exported through `visibility/mod.rs`; the extra `scope.is_empty()` arm moved up into `classify_no_facade_callers` as two explicit checks.
- Phase 11's defect 1 (fix facts copied after suppression) now has **four** suppression stages to prune after, not three.
...
- **Phase 11** — its second pending decision now names a concrete insertion point: all four suppression stages run inside `reconcile_cross_target_reports`, which executes before the fix-fact copies.
- *(user decision, approved)* Converging the four reconciliation stages was considered and **rejected** — recorded above with the three obstacles, so future passes do not relitigate it. No Phase 12 was added.
...
- Phase 9 has shipped, so the conversion count is now measurable — it just has not been re-derived.
...
*Defect 1 — a suppressed narrowing still leaves a stored fix fact behind,* so `--fix pub-use` can apply a narrowing the caller-aware analysis already rejected. Sites: `src/compiler/persistence/caller_aware.rs:6`, `src/compiler/persistence/intersection.rs`, and `src/compiler/persistence/load.rs:70-71` plus `:232-238`, where the facts are copied unconditionally *after* suppression has run. (This defect was previously recorded against `src/fixes/pub_use_fixes/` paths that do not exist — that directory holds only `mod.rs`, `parent_boundary.rs`, `scan.rs`, `validated_plan.rs`.)

*Defect 2 — `pub_use_fixes/scan.rs` requires the literal `"pub "`,* so a declaration written `pub\nstruct Thing` is advertised as fixable and then silently skipped. The literal is `line_contains_plain_pub` at `scan.rs:234`, reached from `screen_candidate` at `:173`. Same class of problem as the corrected paragraph above.
...
- `src/compiler/persistence/{caller_aware.rs, intersection.rs, visibility_priority.rs, visibility_constraint.rs, load.rs}` — defect 1: fix facts are copied unconditionally after suppression (`load.rs:89-98` and `:243-244`). **After Phase 9 there are four suppression stages, not three**, and they all run inside one function — `reconcile_cross_target_reports` (`load.rs:113-119`), called at `:82`, *before* the fact copies. That gives the "prune once after suppression" boundary in the second pending decision a single obvious home that did not exist when it was written. **The `def_path_is_descendant` unification this row used to schedule is already done** — Phase 9's consolidation left exactly one copy, `pub(in crate::compiler) fn def_path_is_descendant` at `policy.rs:514`, re-exported through `visibility/mod.rs`; the extra `scope.is_empty()` arm moved up into `classify_no_facade_callers` as two explicit checks. Do not go looking for a second copy
...
- Decide whether the new `fixability` string requires a `FINDINGS_SCHEMA_VERSION` bump (`src/compiler/constants.rs:65`; **Phase 9 has taken it to `21`, so a bump here means `22`**). A cache written by an older mend has no such value, so only newly written reports carry it — but the plan's own invariant is that a change to emitted findings bumps the version.
- Decide whether the three fields the fixer needs on `reporting::Finding` (`visibility_annotation`, `narrower_scope_def_path`, `item_def_path`) are internal-only and skipped by serde, or public JSON additions with upgrade-contract entries. The fixer needs them internally; exposing them publicly is not required for the edit.
- **Decide the shape of `visibility_annotation` on `reporting::Finding`, not just its visibility.** On `StoredFinding` it is `Option<String>` (`schema.rs:68`). The fixer's decision is three-way — bare `pub` (rewritable), already-restricted (never fixable, per the branch-specific eligibility rule above), and no annotation recorded — so a bare `Option<String>` conflates "private" with "not captured" and forces the fixer to re-parse a string to recover a distinction the compiler already had. The project's type-design rule ("strongly avoid `Option<T>` in domain-owned types and APIs; convert at the boundary into a domain type whose name and variants state what presence or absence means") makes the `StoredFinding → Finding` conversion in `persistence/load.rs` the right conversion point.

Recommendation:
Add `FixSupport::RestrictedAnnotation`, add the matching `FixKind`, bump the schema version to `22`, and keep all three plumbed fields internal with explicit serde skip attributes. Convert `Option<String>` into a named three-variant enum at the `StoredFinding → Finding` boundary so the fixer matches on states rather than re-parsing text. The bump is cheap — it invalidates caches, nothing more — and the fixer does not need to expand the public JSON contract.
...
What exists now:
- **Four** stages can now remove findings before fix facts are copied: intersection, caller-aware suppression, visibility priority, and Phase 9's new visibility-constraint reconciliation. All four run inside `reconcile_cross_target_reports` (`load.rs:113-119`), which executes at `:82` before the copies at `:89-98` and `:243-244` — so "prune once after suppression" has one concrete insertion point, which it did not when this decision was written.
