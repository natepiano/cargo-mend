...
- **Key files:** (all verified; where the design's own line refs were stale the corrected value is given)
  ...
  - `src/compiler/visibility/scan/record.rs` — `:31` `record_visibility_findings`, `:43/:55/:63/:91` secondary-check bails (`:91` = the `visibility_text != "pub"` guard), `:108-115` unused_pub facade/glob suppression, `:141` def-path store, `:164` `record_forbidden_pub_crate`, `:170` exact `"pub(crate)"` match, `:198` suggestion caller, `:215-241` `record_forbidden_pub_in_crate` (`:220` = the `starts_with("pub(in crate::")` hole, `:229` = `DiagnosticCode::ForbiddenPubInCrate`), `:294`/`:345` the two `narrow_to_pub_crate` recorders, `:314` `impl_self_name` use, `:384` `resolve_parent_facade_visibility`, `:401` `parent_facade_exports_item`, `:418` `maybe_record_suspicious_pub`, `:470` def-path store, `:498/:507` `StoredPubUseFixFact` write
  - `src/compiler/visibility/scan/visibility_context.rs` — `ItemCategory { Module, NonModule }` at `:40`, `ItemInfo` at `:45` with `impl_self_name: Option<String>` at `:53`, `FINDINGS_SCHEMA_VERSION` stamping at `:143`
  ...
  - `src/compiler/visibility/field.rs` — 196 lines after Phase 1 (was 214; every pre-Phase-1 line reference in this plan is stale). `use super::annotation::VisibilityReach;` at `:21`; `check_item` at `:30`; the wider-than-type comparison `field_declared.is_strictly_wider(type_declared, ctx.tcx)` at `:100`; `effective_type_visibility` (returns `VisibilityReach`) at `:140-148`. `DefIdVisibility`, the local `visibility_strictly_wider`, and the local `is_at_least` are **gone** — all three now live on `VisibilityReach`
  ...
  - `src/compiler/constants.rs` — `FINDINGS_SCHEMA_VERSION` at `:62` (currently `17`)
  ...
  - `src/reporting/diagnostics.rs` — `DiagnosticSpec` at `:82` (with `headline`), `ForbiddenPubInCrate` spec arm at `:334`, `finding_headline` at `:358`
  ...
  - `tests/diagnostics/{forbidden_pub_crate,allowances,field_visibility_wider_than_type,pub_use_fixes,rendering,narrow_pub_crate,unused_pub,prelude_pub_mod}.rs` — existing suites the fixtures extend. **There is no `forbidden_pub_in_crate.rs` today — it must be created.**
...
- **Invariants:**
  ...
  - **`FINDINGS_SCHEMA_VERSION` is `17`** (`src/compiler/constants.rs:62`). Any change to the persisted shape *or to the meaning of stored values* requires a bump; `load.rs:166-168` rejects on version, analysis fingerprint, or config fingerprint mismatch. Prior-schema reports are rejected, never partially loaded.
...
### Phase 1 — Annotation type and reach algebra · status: done (`6cc7bec`)
...
### Phase 2 — Dynamic diagnostic headlines · status: todo
...
**Acceptance gate:** `verify.sh check`, `verify.sh test`, `verify.sh lint` all green. Existing rendering tests pass unchanged for every `Static` diagnostic. New assertions in `tests/diagnostics/rendering.rs` prove `FindingMessage` reaches the terminal and cargo JSON emits no note duplicating the headline.

---

### Phase 3 — Close the three detection holes · status: todo
...
- `src/compiler/visibility/scan/record.rs` — consume `facade_subject` at `:314`; `resolve_parent_facade_visibility` (`:384`) stops searching by name
...
`record_forbidden_pub_in_crate` (`record.rs:215-241`) becomes: fire unless `pub_in_path` permits the form **and** the annotation's reach equals the chain's required reach **and** the item is a `Declaration`, not a `Use`. Keep `DiagnosticCode::ForbiddenPubInCrate` and its `forbidden-pub-in-crate` anchor.
...
**`suspicious_pub` is the only secondary check that accepts `pub(in ...)`.** `unused_pub`, `suspicious_pub`, and `narrow_to_pub_crate` currently bail on `item.visibility_text != "pub"` (`record.rs:43`, `:55`, `:63`, `:91`).
...
**Files:**
- `src/compiler/visibility/scan/record.rs` — narrowed `record_forbidden_pub_in_crate` (`:215-241`), advice-matrix dispatch, no-facade classifier, `suspicious_pub` admission (`:418`), suggestion caller (`:198`), `StoredPubUseFixFact` guard (`:498`)
...
- `src/reporting/diagnostics.rs` — matrix headlines flow through `HeadlineSource::FindingMessage`
...
**Upgrade contract for `CHANGELOG.md`** — four cases:
...
- `CHANGELOG.md` — the four upgrade cases
