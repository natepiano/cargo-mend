...
### Phase 12 — `required` mode · status: todo
...
**From Phase 11 — the "applied N import fix(es)" wording.** `src/fixes/runner/notices.rs:11-25` counts the `restricted_annotation` scan into the same import-fix total every other non-`pub-use` fixer feeds, so the Required-mode migration this phase schedules will announce N annotation rewrites as N "import fixes". The wording predates Phase 11, but this is the phase that makes it user-visible at scale. **Either fix the notice to name the fix kind, or record an explicit deferral here** — do not leave it unowned a third time. (Note the pinning test at `notices.rs:102`, which asserts the current wording for `field_visibility` and must change with it.)
...
**Pending decision:** the Hana boundary and the default flip still need splitting before dispatch.

Actual problem:
Two of the three boundaries this phase mixes are still bundled: a Hana migration that lives in another repository, and the product decision about changing the compiled-in default. The live Hana checkout has no `mend.toml` and adding one is prohibited, so the experiment needs an isolated configuration path — and a cargo-mend checkpoint commit cannot atomically own another repository's edits either way.

What exists now:
- Phase 7 already emits and tests Required-mode findings locally.
- Phase 9 has shipped, so the conversion count is measurable — it has not been re-derived.
- Phase 11 will supply `--fix` for the conversion, so the migration is mechanical once it lands.
...
What should change:
- Run the Hana experiment under a separate Hana-local plan/checkpoint using an isolated configuration path that never adds `mend.toml` to the source checkout. Drop it from this phase's scope entirely.
- Re-derive the measured conversion set here after Phase 11 lands, then make a separate explicit decision between keeping `permitted`, changing only fresh-install defaults, or migrating existing global values.

Recommendation:
Split the Hana experiment into its own repository-local plan and defer the default choice until the measured conversion set exists. That leaves this phase owning the typed-signature repair, the re-derived count, and the CHANGELOG entry.

Approve this direction, or modify it?

**Pending decision:** the `--fix`-driven migration has no configuration path in this repository, so the phase's stated order cannot run.

Actual problem:
This phase plans to convert cargo-mend's own bare-`pub`-behind-a-facade sites with `cargo mend --fix` and *then* decide about flipping the compiled-in default. That order is unreachable as written. `FixKind::RestrictedAnnotation` is in the default `--fix` selection (`src/config/run_mode.rs:33`, `:56`), but `rewrites_annotation_only` (`src/compiler/visibility/scan/record.rs:1836-1842`) grants `FixSupport::RestrictedAnnotation` only when the *resolved* config is `PubInPath::Required` — and cargo-mend has no `mend.toml` (verified absent). Before the flip, nothing resolves to `Required`, so `--fix` offers zero conversions and the count cannot even be measured. After the flip, the self-policy gate is already red. There is no ordering of the two steps that works without a third mechanism.

What exists now:
- Config precedence is project `mend.toml` > global > compiled-in default (`src/config/loaded.rs:42-51`), and this repo supplies none of the first two.
- The compiled-in default is `Permitted` (`src/config/pub_in_path.rs`).
- There is no CLI flag and no environment variable that overrides `pub_in_path` for a single run.
- The conversion count this phase must re-derive is `summary.fixable_with_fix` from a `Required` run, which inherits the same blocker.

What should change:
Pick one of three buildable mechanisms and name it in the Work Order before dispatch:
- **A temporary repo-local `mend.toml`** pinning `pub_in_path = "required"`, used to measure and to run `--fix`, then deleted in the same phase. Smallest change, no new surface; the risk is a stray commit leaving it behind, and it briefly contradicts the project's no-`mend.toml` stance for this repo.
- **A new CLI or environment override** (e.g. `--pub-in-path=required`) resolved above the project layer. This is genuinely useful beyond this phase — every future policy migration needs the same one-run override — but it is new product surface with its own tests, README, and precedence semantics, and it enlarges the phase.
- **Flip the default first, then fix**, accepting one red self-policy gate between the flip and the conversion. No new mechanism at all, but it violates the plan's own rule that every phase leaves the tree green, and it makes the flip decision before the count that is supposed to inform it exists.

Recommendation:
Take the temporary repo-local `mend.toml`: it is the only option that neither adds product surface nor leaves the tree red, and the phase deletes it before its checkpoint. If the override flag is wanted for its own sake, make it a separate phase — the migration should not be what forces its design.

Approve this direction, or modify it?

**Delegate acceptance gate:** `verify.sh check`, `verify.sh test`, `verify.sh lint`, and `bash ~/.claude/scripts/delegate/verify.sh test cargo-mend diagnostics` green. Keep both 16-finding rendering gates unchanged. **The typed-signature repair changes no test expectations — every existing test must pass unmodified.** The new lib+bin Required-mode fixture asserts both the advertised fix count and that exactly one edit is applied.

**Regression-only — these three behaviors already shipped and this phase must not re-implement them.** Bare `pub` behind a used exact restricted facade staying silent at `"permitted"`, yielding the exact-boundary suggestion at `"required"`, and an already-exact annotation staying silent are each asserted today by `required_setting_reviews_bare_pub_behind_restricted_facade` (`tests/diagnostics/forbidden_pub_in_crate.rs:310`), `fix_leaves_an_accepted_restricted_annotation_alone` (`:407`), and `the_exact_boundary_rewrite_is_offered_only_under_required` (`:456`). `policy.rs` was not in Phase 11's diff, so no Required-mode behavior changed. They appear here as a regression guard on the typed repair, not as work. **This phase's real scope is four items: the typed repair (with the rendered-text sniff and the rename), the re-derived conversion count, the default decision, and the CHANGELOG entry** — plus the `UseSite` one-liner and the lib+bin fixture named in **Files**.
