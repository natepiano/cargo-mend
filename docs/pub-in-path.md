...
Order matters: without it, an item whose callers sit above its parent matches the ordinary row and gets advice producing E0603 — worse than saying nothing. **Where this classifier runs is a Pending decision — see the end of this Work Order.** The use-site map named here belongs to the persistence layer, which runs after the messages this classifier must shape are already built.
...
- `src/compiler/persistence/caller_aware.rs` — expose the caller union for the classifier (`:6`) — **only if the Pending decision below selects the persistence route**
- `src/compiler/visibility/scan/visibility_context.rs` — `sink.use_sites` (`:76`), the in-pass alternative for the classifier
...
**Pending decision:** where the ordered no-facade caller classifier actually runs.

Actual problem:
This Work Order says the classifier's inputs are the persisted use-site map that `apply_caller_aware_suppression` (`compiler/persistence/caller_aware.rs:6`) reads. **It cannot run there.** That function is invoked from `persistence/load.rs:70`, which is *after* the compiler pass has already built every finding's `message` and `suggestion` in `record.rs`. The persistence layer only retains or drops whole findings (`caller_aware.rs:19-31`); it never rewrites one. The classifier's entire job is to choose among three different `suggestion` strings, so running it there is structurally impossible. As written, a fresh implementer discovers this mid-phase.

What exists now:
- `caller_aware.rs:19-31` filters a finding vector; it has no path to a finding's text.
- `sink.use_sites` (`visibility/scan/visibility_context.rs:76`) holds the same kind of caller data *during* the pass, at the point where `record.rs` is composing suggestions — but it covers the **current crate target only**, so for a package with both a lib and one or more bins it sees one target's callers at a time.

What should change:
- Either (a) run the classifier in-pass off `sink.use_sites`, accepting that a caller living in a sibling target is not visible to it; or (b) add a finding-rewriting step to `persistence/load.rs` that recomputes the suggestion after the cross-target union exists, which means moving suggestion text out of `record.rs` for this one diagnostic.

Recommendation:
Take (a) — in-pass off `sink.use_sites` — **and state the lib+bins limitation explicitly in the Work Order** so it is a known, documented gap rather than a surprise. Option (b) splits one diagnostic's message construction across two layers for a case (an item with no facade whose only external caller lives in a sibling target) that the ordered classifier's third branch already handles safely by emitting no annotation replacement.
...
Actual problem:
This phase's case table (`Required` + bare `pub` + exact restricted facade → `suspicious_pub` with a dynamic suggestion) and Phase 10 both require setting `finding.suggestion` on a `SuspiciousPub` finding. That field is `None` for every `suspicious_pub` finding today (`src/compiler/visibility/scan/record.rs:553`), so no code path has ever had both a custom suggestion and the spec's static help in play at once. `SUSPICIOUS_PUB` carries `inline_help: Some("consider using: `pub(super)`")` (`src/reporting/diagnostics.rs:123`) — advice that is wrong for an item sitting behind a facade, where the correct annotation names the facade's boundary.
...
**Phase 4 is direct evidence for splitting.** It was a comparable single-mechanism phase and it took **eleven** fix passes to reach an approving review — and rounds 9, 10, and 11 were spent *entirely* on rendered wording (which modifier a diagnostic may quote, and when). That is exactly 7b's content, in a phase that carried none of 7a's work alongside it.
...
**Changed input from Phase 6 — weigh this before answering.** The recommendation to split rests on Phase 4's eleven fix passes and Phase 5's fourteen as evidence that a wording-heavy phase does not converge in one dispatch. Phase 6 identified a different cause: every one of those passes handed a fresh blind reviewer the *entire accumulated phase diff*, and a competent reviewer always returns something, so "review comes back clean" was unreachable regardless of phase size. Phase 6 scoped its review to the incremental diff and got the run's first clean review on the first pass. That makes **"keep Phase 7 whole, review incrementally"** a third option with evidence behind it, weighed against the renumbering churn splitting would impose on Phases 8–11.
...
- `CHANGELOG.md` — the eleven upgrade cases above, **plus a twelfth**
...
The four predicates behind `assess_signature_exposure_allowance` (`policy.rs:320`) **are no longer in `policy.rs`** — Phase 4 moved them into `src/compiler/exposure/`, re-exported from `exposure/mod.rs:4-8` as `child_item_is_exposed_by_other_crate_visible_signature`, `impl_item_is_exposed_by_exported_self_type`, `child_item_is_exposed_by_sibling_boundary_signature`, and `parent_boundary_public_signature_exposes_child_used_outside_parent`. Each still returns on first match and collapses recursion to `bool`.
...
**Files:**
- `src/compiler/exposure/mod.rs` — **the four predicates now live behind this module** (`:4-8`); they and `assess_signature_exposure_allowance` (`policy.rs:320`) return `Option<VisibilityReach>`
...
- `src/compiler/persistence/{caller_aware.rs, intersection.rs, load.rs}` — defect 1: `load.rs:70-71` and `:232-238` copy fix facts unconditionally after suppression
