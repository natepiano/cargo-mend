...
Then wire `suspicious_pub`'s facade arm from `FixSupport::None` to the new variant so the fix rewrites `pub` to the exact annotation. The fix must also confirm the facade line itself needs no edit before applying — that is why this is last.
