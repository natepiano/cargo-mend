...
- **Key files:** (all verified; where the design's own line refs were stale the corrected value is given)
  ...
  - `src/config/global.rs` — `reconcile_global_config` at `:79`, `GlobalConfig` at `:29`, `GlobalConfigFile` at `:35`
...
- `CHANGELOG.md` — the eleven upgrade cases above
...
**Files:**
- `src/compiler/visibility/policy.rs` — `assess_parent_facade_usage` (`:290`), the `Required` branch
